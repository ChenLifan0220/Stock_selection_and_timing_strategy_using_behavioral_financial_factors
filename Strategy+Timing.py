...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 21 17:52:39 2018
...
CO_factors=pd.read_csv("/Users/lilychen/Desktop/dissertation/Results/CO_factors_ZZ100.csv",sep=",",index_col=0)
...
monthly_sequence=CO_factors.index.tolist()

Selected_stocks_monthly=pd.DataFrame()
for m in monthly_sequence:
#for m in [20071130]:
    single_month_COs=CO_factors.loc[m]
    ...
    Group_A_selected=Group_A.iloc[:int(Group_A.shape[0]*0.3)]
    ...
    for i in selected_stocks_monthly:
        if single_month_COs.loc[i]<0:
            selected_stocks_monthly.remove(i)
...
Selected_stocks_monthly.to_csv("/Users/lilychen/Desktop/dissertation/Results/Selected_stocks_monthly_ZZ100.csv",index=False)
